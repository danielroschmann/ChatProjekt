...
export const createUser = async (req, res) => {
  ...
  if (inputIsBlank(username, password)) {
    return res.render("registerView", {
      errorMessage: "Indtast venligst et brugernavn og et kodeord",
    });
  }
  if (password.length < 8) {
    return res.render("registerView", {
      errorMessage: "Kodeordet er for kort",
    });
  }
  if (password.length > 20) {
    return res.render("registerView", {
      errorMessage: "Kodeordet er for langt",
    });
  }
  if (!usernameIsValid(username)) {
    return res.render("registerView", {
      errorMessage: "Brugernavnet er allerede taget",
    });
  }
