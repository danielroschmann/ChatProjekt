...
export const handleMessageUpdate = async (updatedMessage) => {
  ...
  chat.beskeder = chatMessages.filter(
    (m) => String(m.id) !== String(updatedMessage.id)
  );
  chat.beskeder.push(updatedMessage);

  handleChatUpdate(
    chat,
    chatMessages.findIndex((c) => c.id === chat.id)
  );
...
export const handleMessageCreation = async (message, chatId, username) => {
  console.log(
    `Attempting to create message with message: ${message}, chatId: ${chatId}, and username: ${username}`
  );
  ...
  console.log(
    `Attempting to save message to beskedArr: ${JSON.stringify(beskedArr)}`
  );
